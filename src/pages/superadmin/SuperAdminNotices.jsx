...
import { Bell, Plus, Edit2, Trash2, X, Loader, AlertTriangle, Info, Megaphone, Globe } from "lucide-react";
  ...
  return (
    <div className={`card border ${cfg.cls} space-y-2`}>
      ...
      <div className="flex items-center gap-3 text-xs text-slate-600">
        <span className="flex items-center gap-1"><Globe size={11} /> Global</span>
...
export default function SuperAdminNotices() {
  ...
  const handleCreate = async (form) => {
    setSaving(true);
    try { await api.post("/notices/superadmin", form); setShowCreate(false); load(); }
    ...
    <div className="space-y-5">
      <div className="page-header">
        <div>
          ...
          <p className="page-sub">Post global notices visible to all users across every institute.</p>
        </div>
      ...
      {showCreate && (
        <Modal title="Post Global Notice" onClose={() => setShowCreate(false)}>
