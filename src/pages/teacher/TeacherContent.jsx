  ...
  return (
    <div className="card hover:border-border-2 transition-colors">
      {ytId ? (
        ...
        <div className={`w-9 h-9 rounded-lg flex items-center justify-center mb-3 ${config.bg}`}>
      ...
      <div className="flex items-start justify-between gap-2">
        ...
        <span className={`badge shrink-0 gap-1 ${config.bg} ${config.color} border-0`}>
          <Icon size={10} /> {config.label}
        </span>
      </div>
      {item.type !== "video" && (
        <a href={item.url} target="_blank" rel="noopener noreferrer"
          className={`inline-flex items-center gap-1.5 text-xs font-medium mt-3 ${config.color} hover:underline`}>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="page-sub">Read-only view of content for your assigned class levels.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="card">
          <div className="empty-state py-12">
            ...
            <p className="empty-title">No content for your levels</p>
            <p className="empty-desc">Content will appear here once the SuperAdmin uploads material for your class levels.</p>
          </div>
        </div>
      ) : (
