...
import { KeyRound, User, Loader, CheckCircle } from "lucide-react";
  ...
  return (
    <div className="space-y-6 max-w-lg">
      <div>
        ...
        <p className="page-sub">Manage your profile and account security.</p>
      </div>
      ...
      <div className="card">
        <div className="flex items-center gap-4 mb-4">
          <div className="w-12 h-12 rounded-lg flex items-center justify-center font-bold text-lg text-white flex-shrink-0"
            ...
            {user?.fullName?.split(" ").slice(0, 2).map((n) => n[0]).join("") || "U"}
          </div>
          <div>
            <div className="text-sm font-semibold text-slate-200">{user?.fullName}</div>
            <div className="text-xs text-slate-500">{user?.email}</div>
            <div className="text-xs text-slate-600 capitalize mt-0.5">{user?.role}</div>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-3 text-xs">
          ...
          {user?.rollNumber && (
            <div className="bg-surface rounded-lg p-3">
              <div className="text-slate-500 mb-0.5">Roll No.</div>
              <div className="text-slate-300">{user.rollNumber}</div>
            </div>
          )}
        </div>
      </div>
