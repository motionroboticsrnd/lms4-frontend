...
import { Bell, Plus, Edit2, Trash2, X, Loader, AlertTriangle, Info, Megaphone, Building2 } from "lucide-react";
...
function NoticeForm({ initial, onSave, onClose, saving }) {
  const [form, setForm] = useState(initial || { title: "", content: "", type: "info", expiresAt: "" });
      ...
      <div className="space-y-4">
        <div className="form-group">
          ...
          <input className="input" value={form.title} onChange={(e) => set("title", e.target.value)}
            placeholder="e.g. Parent-Teacher Meeting on Monday" />
        </div>
        <div className="form-group">
  ...
  return (
    <div className={`card border ${cfg.cls} space-y-2`}>
      ...
      <div className="flex items-center gap-3 text-xs text-slate-600">
        <span className="flex items-center gap-1"><Building2 size={11} /> School-wide</span>
...
export default function AdminNotices() {
  ...
  const handleCreate = async (form) => {
    setSaving(true);
    try { await api.post("/notices/admin", form); setShowCreate(false); load(); }
    ...
    <div className="space-y-5">
      <div className="page-header">
        <div>
          ...
          <p className="page-sub">Post notices visible to all teachers and students in your school.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="card py-12 text-center">
          ...
          <p className="text-sm text-slate-500">No notices yet. Click "Post Notice" to create one.</p>
        </div>
      ) : (
      ...
      {showCreate && (
        <Modal title="Post School Notice" onClose={() => setShowCreate(false)}>
          <NoticeForm onSave={handleCreate} onClose={() => setShowCreate(false)} saving={saving} />
        </Modal>
      )}
      {editTarget && (
        <Modal title="Edit Notice" onClose={() => setEditTarget(null)}>
          <NoticeForm initial={editTarget} onSave={handleEdit} onClose={() => setEditTarget(null)} saving={saving} />
        </Modal>
      )}
    </div>
  );
